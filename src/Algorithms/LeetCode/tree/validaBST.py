...
def isvalidbsttree3(node):
	if not node:
		return True

	valid_shape = [(node, float('-inf'), float('inf'))]

	while valid_shape:
		root, lower, upper = valid_shape.pop()
		if not root:
			continue
		...
		valid_shape.append((root.left, lower, val))
		valid_shape.append((root.right, val, upper))
